...
const getRoomById = async (id) => {
    try {
        const chosenRoom = await db.any('SELECT * FROM meeting_room WHERE id = $1', [id]);
        return chosenRoom;
    } catch (error) {qq
        console.log(error.message || error);
        return null;  
...
const getFutureBookingsForRoom = async (roomId) => {
    try {
        const futureBookings = await db.any(`
            ...
            ORDER BY start_date ASC`, [roomId]);
...
module.exports = {getRooms, createRoom, getRoomById,getFutureBookingsForRoom}